...
const apibase = 'https://v1.formula-1.api-sports.io';
...
(function ($) {
    ...
    var settings = {
        "url": apibase+"/races?date=" + today,
        "method": "GET",
        "timeout": 0,
        "headers": {
            "x-rapidapi-key": apikey,
            "x-rapidapi-host": "formula-1.api-sports.io"
        },
    };
    $.ajax(settings).done(function (response) {
        data = response;
        console.log(data);
        data.response.forEach(function (single_race) {
            var rtype = single_race.type;
            var rstat = single_race.status;
            var cname = single_race.competition.name;
            var ccountry = single_race.competition.location.country;
            var ccity = single_race.competition.location.city;
            var raceitem = '<div>'+cname+' ('+rtype+')'+', '+ccity+','+ccountry+'<br>'+rstat+'</div>';
            $('.gamescontainer').append(raceitem);
        });
    });
