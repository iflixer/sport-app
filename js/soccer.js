...
$(document).ready(function () {
    var apptgid = $('body').data('tgid');
    apptgid = +apptgid;
    var apptgname = $('body').data('tgname');
    getAT(apptgid,apptgname,'',false,false);
});

    const soccerbase = 'https://api.flashscore.ai/api/soccer';
...
function soccerdata(a_t) {
        ...
        var settings = {
            "url": soccerbase + "/fixtures?season=" + currentseason + "&date=" + today,
            "method": "GET",
            "timeout": 0,
        ...
        function getLeagueOdds(league, inplay) {
            ...
            // ODDS SETTINGS
            var settings2 = {
                "url": soccerbase + '/odds?league=' + league + '&season=' + currentseason + '&date=' + today,
                "method": "GET",
                "timeout": 0,
                contentType: 'application/json',
                headers: {
            ...
            $.ajax(settings2).done(function (response) {
                ...
                $.each(response.data.response, function (index, item) {
                    ...
                    if (fstat === 'LIVE' || fstat === '1H' || fstat === 'HT' || fstat === '2H' || fstat === 'ET' || fstat === 'P') {
                        ...
                        var settings3 = {
                            "url": soccerbase + '/odds/live?fixture=' + fixid,
                            "method": "GET",
                            "timeout": 0,
                            headers: {
                                'Authorization': 'Bearer '+ a_t
